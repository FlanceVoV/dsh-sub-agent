 ...
 *  - lib/src/team.js      团队模式：群组讨论的驱动循环、群主调度、把分工派成任务链（唯一碰「谁在说话」处）。
...
import { createTeamBoard } from './src/team.js';
...
export function apply(ctx, entryConfig) {
  ...
  if (context.store !== undefined) {
    ...
    // 团队模式（群组）：构造顺序在 tasks 之后是硬性的——群主的「派活」要落成任务链，
    // 而讨论驱动要等运行结算才知道该不该把话语权还给群主。
    try {
      context.team = createTeamBoard({
        ctx,
        store: context.store,
        runtime: context.runtime,
        tasks: context.tasks,
      ...
      ctx.effect(() => context.team.attach(), 'subagent-hub: team board');
    } catch (error) {
      context.team = undefined;
      log.warn(`团队模式未启用（其余能力不受影响）：${describeError(error)}`);
    }
  }
  ...
  log.info(
    ...
    + `team=${context.team === undefined ? 'unavailable' : (context.team.isTeamMode() ? 'on' : 'off')}`,
  );
}
