...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 面板尺寸的上下限：拖拽缩放时按它夹取（不让人把面板拖成一条缝或拖出屏幕）。 */
    const PANEL_MIN_W = 300;
    const PANEL_MIN_H = 220;
    /** 展开时的默认尺寸：默认就比原来宽一些——横向依赖图需要宽度才看得出并行分叉。 */
...
/* 卡片是纵向 flex，内部元素才能「跟着卡片变高」而不是各管各的（缩放时最明显）。
   宽度由 JS 按面板状态给（可拖拽缩放），所以这里不再写死宽度。
   position:relative 是为了让右下角的缩放手柄相对**卡片**定位——
   否则它会去认外层那个 fixed 的 .sbh-layer，视觉上凑巧对得上，一改布局就错位。 */
.sbh-card{display:flex;flex-direction:column;position:relative;max-height:calc(100vh - 20px)}
.sbh-head{display:flex;align-items:center;gap:8px;padding:9px 12px;cursor:grab;user-select:none;touch-action:none;flex:none}
...
/* 面板主体：默认给一个舒服的高度上限；用户拖大之后由行内 height 接管（flex:1 撑满）。 */
.sbh-body{border-top:1px solid color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 9%,transparent);
  max-height:min(64vh,760px);overflow:auto;overscroll-behavior:contain;flex:1 1 auto;min-height:0}
/* 右下角的缩放把手：只有它自己拦截指针事件，卡片其余部分照旧可以拖动。 */
.sbh-resize{position:absolute;right:0;bottom:0;width:18px;height:18px;cursor:nwse-resize;touch-action:none;
  opacity:.5;z-index:2}
.sbh-resize:hover{opacity:.95}
.sbh-resize__grip{position:absolute;right:3px;bottom:3px;width:10px;height:10px;pointer-events:none;
  border-right:2px solid var(--dsw-alias-label-tertiary,#9a9a9a);
  border-bottom:2px solid var(--dsw-alias-label-tertiary,#9a9a9a);border-bottom-right-radius:3px}
...
/* 面板拉宽之后，「下一步」这种整句提示要能折行——一行截断会把最关键的后半句吃掉。 */
.sbh-sub--wrap{white-space:normal;overflow:visible;text-overflow:clip;line-height:1.5}
...
.sbh-badge--warn{color:var(--dsw-alias-state-warn-label,#a35b00);
  background:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#ffaa00) 18%,transparent);
  border-color:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#ffaa00) 40%,transparent)}
.sbh-badge--error{color:var(--dsw-alias-state-error-primary,#e5484d);
  background:color-mix(in srgb,var(--dsw-alias-state-error-primary,#e5484d) 14%,transparent);
  border-color:color-mix(in srgb,var(--dsw-alias-state-error-primary,#e5484d) 36%,transparent)}
...
/* ---- 团队模式下拉（0.3.0）----
   紧挨着启用开关放，但**只在插槽给得出位置时**才有一席之地：
   用原生 <select> 而不是自绘菜单——它只表达「开 / 关」两态，
   自绘要多维护一份焦点、键盘与点击外部关闭的行为，这里没有任何一项值那个成本。 */
.sbh-toggle-group{display:inline-flex;align-items:center;gap:4px;min-width:0}
.sbh-toggle__select{height:24px;max-width:150px;padding:0 4px;font:inherit;font-size:11.5px;cursor:pointer;
  color:var(--dsw-alias-label-secondary,#5b5b5b);background:transparent;
  border:1px solid color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 14%,transparent);border-radius:8px}
.sbh-toggle__select:hover{background:color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 8%,transparent)}
.sbh-toggle__select:disabled{opacity:.55;cursor:default}
.sbh-toggle__select:focus{outline:2px solid color-mix(in srgb,var(--dsw-alias-interactive-bg-hover,#4d6bfe) 45%,transparent);
  outline-offset:-1px}
.sbh-toggle__select--on{color:var(--dsw-alias-brand-primary,#2f6fed);
  border-color:color-mix(in srgb,var(--dsw-alias-brand-primary,#2f6fed) 40%,transparent);
  background:color-mix(in srgb,var(--dsw-alias-brand-primary,#2f6fed) 12%,transparent)}
/* ---- 群组（团队模式，0.3.0）----
   群聊是**读得多、看得快**的东西：一条发言一行，说话人用色块区分，
   正文左对齐、允许折行但不允许撑破面板（min-width:0 一路给到底）。 */
.sbh-paneltabs{display:flex;gap:4px;padding:7px 12px 0;flex:none}
.sbh-team{display:flex;flex-direction:column;flex:1 1 auto;min-height:0}
.sbh-team__head{display:flex;align-items:center;gap:6px;padding:7px 12px 4px;min-width:0;flex-wrap:wrap}
.sbh-team__name{font-weight:650;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sbh-team__meta{font-size:10.5px;color:var(--dsw-alias-label-tertiary,#9a9a9a);white-space:nowrap}
.sbh-team__members{display:flex;flex-wrap:wrap;gap:4px;padding:2px 12px 6px}
.sbh-chip{display:inline-flex;align-items:center;gap:4px;padding:1px 7px;border-radius:999px;font-size:10.5px;
  color:var(--dsw-alias-label-secondary,#5b5b5b);
  background:color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 7%,transparent);
  border:1px solid transparent;white-space:nowrap}
.sbh-chip--owner{color:var(--dsw-alias-brand-primary,#2f6fed);font-weight:600;
  border-color:color-mix(in srgb,var(--dsw-alias-brand-primary,#2f6fed) 34%,transparent)}
.sbh-chip--busy{color:var(--dsw-alias-state-success-primary,#0a7b45);
  border-color:color-mix(in srgb,var(--dsw-alias-state-success-primary,#00be6e) 34%,transparent)}
.sbh-team__plan{padding:2px 12px 8px;display:flex;flex-direction:column;gap:5px;
  border-bottom:1px solid color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 7%,transparent)}
.sbh-team__planrow{display:flex;align-items:center;gap:7px;flex-wrap:wrap}
.sbh-team__log{flex:1 1 auto;min-height:0;overflow:auto;overscroll-behavior:contain;padding:8px 12px}
.sbh-msg{display:flex;flex-direction:column;gap:1px;padding:5px 7px;margin-bottom:4px;border-radius:9px;
  background:color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 5%,transparent);
  border-left:2px solid color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 22%,transparent)}
.sbh-msg--owner{border-left-color:var(--dsw-alias-brand-primary,#2f6fed);
  background:color-mix(in srgb,var(--dsw-alias-brand-primary,#2f6fed) 9%,transparent)}
.sbh-msg--user{border-left-color:var(--dsw-alias-state-warn-primary,#ffaa00);
  background:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#ffaa00) 10%,transparent)}
.sbh-msg--system{background:transparent;border-left-style:dashed}
.sbh-msg--overlong{border-left-color:var(--dsw-alias-state-error-primary,#e5484d)}
.sbh-msg__who{display:flex;align-items:center;gap:6px;font-size:10px;font-weight:700;
  color:var(--dsw-alias-label-secondary,#5b5b5b);min-width:0}
.sbh-msg__text{font-size:11.5px;line-height:16.5px;white-space:pre-wrap;word-break:break-word}
.sbh-team__compose{display:flex;flex-direction:column;gap:6px;padding:7px 12px 9px;flex:none;
  border-top:1px solid color-mix(in srgb,var(--dsw-alias-label-primary,#101010) 9%,transparent)}
.sbh-team__row{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.sbh-team__hint{font-size:10.5px;line-height:15px;color:var(--dsw-alias-label-tertiary,#9a9a9a)}
/* ---- 收起态：玻璃水球 ----
...
/* 线型图例：画真线（实/虚/流动三种线型只能靠看，不能靠说）。 */
.sbh-legend--edges{margin-top:-2px}
.sbh-legend__line{flex:none;width:26px;height:8px}
.sbh-legend__line .sbh-edge{stroke-width:1.8}
...
/* 图不再自己限高：面板可以拖高，图就该跟着长（外面的 .sbh-body 才是滚动容器）。
   以前这里写死 max-height:46vh，结果是把面板拖得再高，图也还是那么矮、还多出一条内滚动条。 */
.sbh-graph__scroll{overflow:auto;max-width:100%;overscroll-behavior:contain}
...
/* ⚠ 节点里的文字**必须显式给 fill**。SVG 的默认填充是黑色，而节点底色用的是主题的
   bg-layer-1：深色主题下那就是深底 + 黑字，等于看不见。
   这里统一用 currentColor 跟随卡片本身的文字颜色（DSH 已经按主题算好了），
   再带一层 DSH 自己的 boot 兜底变量——这是它自家组件里在用的写法。
   ⚠ 再次提醒：本样式表是模板字符串，注释里**不能出现反引号**（踩过两次了）。 */
.sbh-graph{color:var(--dsw-alias-label-primary, var(--dsh-boot-label-primary, #1a1a1a))}
.sbh-node__id{font-family:var(--dsw-font-mono,ui-monospace,Menlo,Consolas,monospace);font-weight:700;fill:currentColor;opacity:.75}
.sbh-node__title{font-weight:600;fill:currentColor}
.sbh-node__meta{font-family:var(--dsw-font-mono,ui-monospace,Menlo,Consolas,monospace);fill:currentColor;opacity:.62}
.sbh-node__chip{font-weight:600;fill:currentColor;opacity:.85}
...
.sbh-node--waiting .sbh-node__box{stroke-dasharray:3 3;
  ...
  fill:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#fff) 62%,transparent)}
.sbh-node--waiting .sbh-node__accent{fill:color-mix(in srgb,currentColor 30%,transparent)}
.sbh-node--waiting .sbh-node__title{opacity:.68}
.sbh-node--waiting .sbh-node__chip{opacity:.5}
.sbh-node--ready .sbh-node__box{stroke:var(--dsw-alias-brand-primary,#2f6fed);stroke-width:1.5;
  fill:color-mix(in srgb,var(--dsw-alias-brand-primary,#2f6fed) 12%,var(--dsw-alias-bg-layer-1,#fff))}
.sbh-node--ready .sbh-node__accent{fill:var(--dsw-alias-brand-primary,#2f6fed)}
.sbh-node--ready .sbh-node__glow{stroke:var(--dsw-alias-brand-primary,#2f6fed);animation:sbh-breathe 2.6s ease-in-out infinite}
.sbh-node--ready .sbh-node__chip{fill:var(--dsw-alias-brand-primary,#2f6fed);opacity:1}
.sbh-node--running .sbh-node__box{stroke:var(--dsw-alias-state-success-primary,#00be6e);stroke-width:1.6;
  fill:color-mix(in srgb,var(--dsw-alias-state-success-primary,#00be6e) 14%,var(--dsw-alias-bg-layer-1,#fff))}
...
.sbh-node--running .sbh-node__chip{fill:var(--dsw-alias-state-success-primary,#00be6e);opacity:1}
.sbh-node--done .sbh-node__box{stroke:color-mix(in srgb,var(--dsw-alias-state-success-primary,#00be6e) 45%,transparent);
  fill:color-mix(in srgb,var(--dsw-alias-state-success-primary,#00be6e) 11%,var(--dsw-alias-bg-layer-1,#fff))}
.sbh-node--done .sbh-node__accent{fill:color-mix(in srgb,var(--dsw-alias-state-success-primary,#00be6e) 70%,transparent)}
.sbh-node--done .sbh-node__chip{fill:var(--dsw-alias-state-success-primary,#00be6e);opacity:1}
.sbh-node--failed .sbh-node__box{stroke:var(--dsw-alias-state-error-primary,#e5484d);
  fill:color-mix(in srgb,var(--dsw-alias-state-error-primary,#e5484d) 13%,var(--dsw-alias-bg-layer-1,#fff))}
.sbh-node--failed .sbh-node__accent{fill:var(--dsw-alias-state-error-primary,#e5484d)}
.sbh-node--failed .sbh-node__chip{fill:var(--dsw-alias-state-error-primary,#e5484d);opacity:1}
.sbh-node--blocked .sbh-node__box{stroke:var(--dsw-alias-state-warn-primary,#ffaa00);
  fill:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#ffaa00) 15%,var(--dsw-alias-bg-layer-1,#fff))}
.sbh-node--blocked .sbh-node__accent{fill:var(--dsw-alias-state-warn-primary,#ffaa00)}
.sbh-node--blocked .sbh-node__chip{fill:var(--dsw-alias-state-warn-primary,#ffaa00);opacity:1}
.sbh-node--cancelled .sbh-node__box{stroke:color-mix(in srgb,currentColor 26%,transparent);
  fill:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#fff) 62%,transparent)}
.sbh-node--cancelled .sbh-node__accent{fill:color-mix(in srgb,currentColor 30%,transparent)}
.sbh-node--cancelled .sbh-node__title{opacity:.5;text-decoration:line-through}
.sbh-node--cancelled .sbh-node__chip{opacity:.45}
...
`;
     ...
     * 拖拽缩放的几何（纯函数，便于断言）。
     *
     * ⚠ 这里曾经算错过一次，症状是「**只有上下能拖、左右拖不动**」，而且很难自己看出来：
     * 当时宽度用的是「指针到卡片右边界的距离」。可把手本来就贴在卡片右边界上，
     * 于是拖拽第一帧算出来 ≈ 0 → 立刻被夹到最小宽度；之后要往左拖将近 300px 才开始变宽。
     * 高度用的是**位移**（`start + dy`），所以只有上下正常。
     *
     * 结论：两个轴必须用同一种算法——**相对于拖拽起点的位移**；
     * 并且拖拽一开始就把面板钉成左上定位（x/y 明确），把手才会一直跟着指针，
     * 不会出现「卡片底部贴边、往下拖却不动」的别扭手感。
     *
     * @param {{startWidth:number,startHeight:number,startLeft:number,startTop:number,dx:number,dy:number,viewportWidth:number,viewportHeight:number}} input - 起点与位移。
     * @returns {{width:number,height:number,x:number,y:number}} 新的尺寸与位置。
     */
    function resizePanelBox(input) {
      const left = Math.max(0, Number(input.startLeft) || 0);
      const top = Math.max(0, Number(input.startTop) || 0);
      // 上限跟着视口算：拖到屏幕外的部分不是「更大的面板」，只是看不见的溢出。
      const maxWidth = Math.max(PANEL_MIN_W, Number(input.viewportWidth) - 24 - left);
      const maxHeight = Math.max(PANEL_MIN_H, Number(input.viewportHeight) - 24 - top);
      return {
        width: Math.round(clampNumber(Number(input.startWidth) + Number(input.dx), PANEL_MIN_W, maxWidth)),
        height: Math.round(clampNumber(Number(input.startHeight) + Number(input.dy), PANEL_MIN_H, maxHeight)),
        x: Math.round(left),
        y: Math.round(top),
      };
    }
    ...
    function loadPanelState() {
      const fallback = { expanded: false, side: "right", x: null, y: null, width: PANEL_DEFAULT_W, height: PANEL_DEFAULT_H };
      try {
        ...
        return {
          ...
          width: clampPanelSize(parsed?.width, PANEL_MIN_W, PANEL_DEFAULT_W, "w"),
          height: clampPanelSize(parsed?.height, PANEL_MIN_H, PANEL_DEFAULT_H, "h"),
        };
      } catch {
        return fallback;
      }
     ...
     * 夹取面板尺寸。
     *
     * 上限跟视口走而不是写死：把面板拖到 1200px 之后换到小屏（或用户把窗口缩小），
     * 写死的上限会让面板比窗口还宽，界面上就只剩一个关不掉的横条。
     * @param {unknown} value - 目标值。
     * @param {number} min - 下限。
     * @param {number} fallback - 取不到值时的默认。
     * @param {'w'|'h'} axis - 轴向（决定跟视口的宽还是高比）。
     * @returns {number}
     */
    function clampPanelSize(value, min, fallback, axis) {
      const viewport = typeof window !== "undefined" && window.innerWidth
        ? (axis === "w" ? window.innerWidth : window.innerHeight)
        : 1200;
      const max = Math.max(min, viewport - 24);
      const num = Number(value);
      if (!Number.isFinite(num)) return Math.min(fallback, max);
     ...
     * 归一化宿主给的团队（群组）负载。
     *
     * 与任务图同理但**更严格**：群聊里每一行都要渲染说话人与正文，
     * 少一个字段就是一行 `undefined` 或一个空白气泡——那比整块不显示更让人困惑。
     * 认不出形状就退回空帧（面板会显示「还没有团队」），而不是把坏数据画出来。
     *
     * @param {unknown} raw - 原始负载。
     * @returns {{teams:object[],total:number,teamMode:boolean,active:number}|undefined}
     */
    function normalizeTeamFrame(raw) {
      ...
      for (const item of Array.isArray(raw.teams) ? raw.teams : []) {
        ...
        teams.push({
          ...
          members: Array.isArray(item.members) ? item.members.filter((name) => typeof name === "string") : [],
          mission: typeof item.mission === "string" ? item.mission : "",
          status: typeof item.status === "string" ? item.status : "",
          phase: typeof item.phase === "string" ? item.phase : "",
          speaker: typeof item.speaker === "string" ? item.speaker : "",
          rounds: Number.isFinite(item.rounds) ? item.rounds : 0,
          maxRounds: Number.isFinite(item.maxRounds) ? item.maxRounds : 0,
          planId: typeof item.planId === "string" ? item.planId : "",
          lastSeq: Number.isFinite(item.lastSeq) ? item.lastSeq : 0,
      ...
      return {
        teams,
        total: Number.isFinite(raw.total) ? raw.total : teams.length,
        teamMode: raw.teamMode === true,
        active: Number.isFinite(raw.active) ? raw.active : 0,
      };
    }
    //#endregion
    ...
    const hub = (() => {
      ...
      let snapshot = {
        ...
        // 团队（群组）与任务图同一帧，见 normalizeTeamFrame 的说明。
        team: { teams: [], total: 0, teamMode: false, active: 0 },
        health: null,
      };
      let source = null;
      ...
      const refresh = async (sessionId) => {
        ...
        try {
          ...
          set({
            ...
            team: normalizeTeamFrame(data.team) ?? snapshot.team,
          });
        } catch (error) {
    ...
    function ComposerToggle(props) {
      ...
      const teamMode = state.team?.teamMode === true;
      const label = enabled
       ...
       * 团队模式开关（0.3.0）。
       *
       * 为什么失败要留话：这一项是**全局**的，拒绝的理由通常很具体（团队服务没装配、
       * 宿主版本旧到没有这个端点）——静默回弹会让用户以为「点了没反应」，
       * 所以把原因留在 title 里，鼠标移上去就能看到。
       */
      const onTeamChange = React.useCallback(async (event) => {
        if (teamBusy) return;
        const next = event.target.value === "on";
        setTeamBusy(true);
        setTeamError("");
        const result = await hub.post("/team/mode", { enabled: next });
        setTeamBusy(false);
        if (!result.ok) {
          setTeamError(result.payload?.error ?? `HTTP ${result.status}`);
        } else if (Array.isArray(result.payload?.paused) && result.payload.paused.length > 0) {
          setTeamError(`已关闭：${result.payload.paused.length} 个正在讨论的团队被暂停（重新打开后点「继续」即可接着走）`);
        }
        await hub.refresh(sessionId);
      }, [teamBusy, sessionId]);

      const teamTitle = teamError !== ""
        ? teamError
        : (teamMode
          ? "团队模式已开：用户消息里声明团队（团队名称/团队负责人/团队成员）就会开一场群聊，讨论与派活由宿主自己推进"
          : "开启后：用户在消息里声明团队，群主主持讨论并直接把分工派成任务链去执行（任务链固定自动激活）");

      return h("span", { className: "sbh-toggle-group" }, [
        h("button", {
          key: "toggle",
          type: "button",
        ...
        // 团队模式下拉：与启用开关并排，但**自己是一个开关**——
        // 它管的是「要不要群聊」，启用开关管的是「子 agent 能不能被调用」。
        h("select", {
          key: "team",
          className: `sbh-toggle__select${teamMode ? " sbh-toggle__select--on" : ""}`,
          value: teamMode ? "on" : "off",
          disabled: teamBusy,
          title: teamTitle,
          "aria-label": t?.("team.mode") ?? "团队模式",
          onChange: onTeamChange,
        }, [
          h("option", { key: "off", value: "off" }, t?.("team.off") ?? "团队模式：关"),
          h("option", { key: "on", value: "on" }, t?.("team.on") ?? "团队模式：开"),
        ]),
      ]);
    }
    //#endregion
    ...
    function SubAgentBall(props) {
      ...
      /**
       * 展开面板的两个页签（0.3.0）：运行态 / 群组。
       *
       * 为什么放在 panelObservable 之外（不落 localStorage）：页签是「我现在想看什么」，
       * 不是「面板长什么样」。刷新页面之后回到「运行态」是更合理的默认——
       * 群组里没新消息时，用户更可能想确认球上那些活儿在不在跑。
       */
       ...
       * 右下角拖拽缩放。
       *
       * 两个要点，都是踩过之后才写对的（完整记录见 `resizePanelBox`）：
       *  1. 尺寸按**相对拖拽起点的位移**算，不是「指针到边界的距离」——后者在把手贴边时恒为 0，
       *     症状就是「只有上下能拖、左右拖不动」。
       *  2. 按下那一刻记下卡片当前的 left/top，拖拽期间把面板**钉成左上定位**：
       *     否则面板底部贴边时，变高只会向上长，把手不跟指针走，拖起来像在推一堵墙。
       *     用的是当前屏幕位置，所以视觉上不会跳。
       */
      const resizeStateRef = React.useRef(null);
      const onResizeDown = React.useCallback((event) => {
        if (event.button !== 0) return;
        // 别让缩放冒泡成「拖动面板」——头部那套拖拽就在同一张卡片里。
        event.stopPropagation();
        event.preventDefault?.();
        const card = event.currentTarget.parentElement;
        const rect = card?.getBoundingClientRect?.();
        // 量不到尺寸时退回面板状态里的值：某些情况下（元素刚挂载、被动画隐藏）
        // rect 会给出 0，而 0 会让「位移 + 0」的起点彻底错掉——那时拖起来就像没抓住。
        const rectWidth = Number(rect?.width) > 0 ? Number(rect.width) : panel.width;
        const rectHeight = Number(rect?.height) > 0 ? Number(rect.height) : panel.height;
        resizeStateRef.current = {
          startX: event.clientX,
          startY: event.clientY,
          width: rectWidth,
          height: rectHeight,
          left: Number.isFinite(rect?.left) && Number(rect.left) >= 0 ? Number(rect.left) : 0,
          top: Number.isFinite(rect?.top) && Number(rect.top) >= 0 ? Number(rect.top) : 0,
        };
        event.currentTarget.setPointerCapture?.(event.pointerId);
      ...
      const onResizeMove = React.useCallback((event) => {
        ...
        const viewportWidth = typeof window !== "undefined" ? window.innerWidth : 1200;
        const viewportHeight = typeof window !== "undefined" ? window.innerHeight : 800;
        const box = resizePanelBox({
          startWidth: start.width,
          startHeight: start.height,
          startLeft: start.left,
          startTop: start.top,
          dx: event.clientX - start.startX,
          dy: event.clientY - start.startY,
          viewportWidth,
          viewportHeight,
        });
        // 位置也一起写回（钉成左上定位），把手才会一直跟着指针。
        panelObservable.set({ ...panelObservable.getSnapshot(), ...box });
      }, []);
      ...
      const teamFrame = state.team ?? { teams: [], teamMode: false, active: 0 };
      // 页签上的数字只数「还没收尾」的团队：收尾的群聊不再需要盯，数进去只会让数字永远降不下来。
      const teamCount = teamFrame.teams.filter((team) => team.status !== "closed").length;
      ...
      return h("div", { className: "sbh-layer", style: showStrip ? stripPosition : position }, [
        ...
        showCard ? h("div", {
          ...
          state.error !== null && state.error !== undefined
            ...
            : null,
          // 页签行：只在展开态、且没有打开某次运行的详情时出现
          // （详情页有它自己的返回按钮，两个导航条同时在场会让人不知道该按哪个回去）。
          panel.expanded === true && openRunId === null
            ? h("div", { key: "tabs", className: "sbh-paneltabs" }, [
              h("button", {
                key: "runs",
                type: "button",
                className: `sbh-tab${tab === "runs" ? " sbh-tab--on" : ""}`,
                onClick: (event) => { event.stopPropagation(); setTab("runs"); },
              }, "运行态"),
              h("button", {
                key: "team",
                type: "button",
                className: `sbh-tab${tab === "team" ? " sbh-tab--on" : ""}`,
                onClick: (event) => { event.stopPropagation(); setTab("team"); },
                title: "团队模式：群主主持的群聊，讨论直接接上任务链",
              }, `群组${teamCount > 0 ? ` ${teamCount}` : ""}`),
            ])
            : null,
          ...
          openRunId !== null
            ...
            : tab === "team"
              ? h(TeamPanel, { key: "team", onOpenRun: (runId) => setOpenRunId(runId) })
...
// 把「插件对外暴露了什么」集中在一处，比每个分片各自往 exports 上挂东西更容易复核。
    ...
    function TeamGroupView(props) {
      ...
      return h("div", { className: "sbh-team" }, [
        ...
        teams.length > 1
          ...
          }, `${team.name}${team.status === "closed" ? "（已收尾）" : ""}`)))
          : null,
    ...
    function TeamPanel(props) {
      ...
      // 默认挑「最需要看的那个」：先看还没收尾的，其次才是最近收尾的。
      const active = teams.find((team) => team.id === selectedId)
        ?? teams.find((team) => team.status !== "closed")
        ?? teams[0]
        ?? null;
    ...
    const en = {
      ...
      "team.mode": "Team mode",
      "team.on": "Team mode: on",
      "team.off": "Team mode: off",
    ...
    // 团队模式（0.3.0，实现见 045-team-group.js）：
    exports.TeamPanel = TeamPanel;
    exports.TeamGroupView = TeamGroupView;
    exports.TeamMessageView = TeamMessageView;
    exports.normalizeTeamFrame = normalizeTeamFrame;
    ...
    exports.__internal = {
      ...
      taskGraph: {
        ...
        TASK_STATE_GLYPH,
        GRAPH_METRICS,
        GRAPH_METRICS_VERTICAL,
        GRAPH_MIN_NODE_W,
        NARROW_GRAPH_W,
      },
      /**
       * 群组的纯函数与词汇表。校验用的负载是**宿主真实产生**的形状
       * （`/team/<id>` 的返回），不是手写的近似值——否则「渲染不炸」证明不了任何事。
       */
      team: {
        TEAM_STATUS_LABEL,
        TEAM_MSG_CLASS,
        TEAM_DECLARATION_TEMPLATE,
        teamClock,
        teamSpeakerLabel,
      },
    };
