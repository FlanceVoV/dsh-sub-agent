...
export function defaultConfig() {
  return {
     ...
     * 所以这个等待是在干正事，不是空转，默认给足 1 小时：一场多角色、多轮的讨论
     * 中途还夹着派出去的任务，本来就可能跑很久，掐太短会让主通道频繁超时。
     * 到点还没谈完就如实回一句「还在跑」，主对话接着用 `team_status {wait:true}` 等。
     *
     * 它同时是**工具参数的 ceiling**：`timeout_ms` / `wait_ms` 只能在这个值之内调短，
     * 想更长就改这个配置——上限由用户定，不由模型定。
     */
    teamWaitMs: 60 * 60 * 1000,
  };
}
...
export const CONFIG_META = Object.freeze({
  ...
  teamWaitMs: {
    ...
    hint: '主对话留在自己的回合里等群主把结论说完（这是结论回到主对话的主通道）。默认 1 小时；工具参数 timeout_ms / wait_ms 只能在这个值之内调短——想更长就改这里。',
  },
});
