...
export function createTeamBoard({ ctx, store, runtime, tasks, config, log }) {
   ...
   * 起/继续一场讨论。
   * @param {string} teamId - 团队。
   * @param {{parentAgent?:object, invokedBy?:string}} [options] - 选项。
    ...
    pendingLines(limit = 5) {
      ...
      for (const team of teams) {
        ...
        lines.push(
          ...
          + `｜${team.status}${team.status === 'discussing' ? `（第 ${team.rounds}/${team.maxRounds} 轮，正在说：@${team.speaker || '…'}）` : ''}`
