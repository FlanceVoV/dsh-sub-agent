...
export function createTeamBoard({ ctx, store, runtime, tasks, config, log }) {
   ...
   *
   * @param {string} teamId - 团队 id。
   * @param {{timeoutMs?:number}} [options] - 等待上限；缺省用 `config.teamWaitMs`（默认 1 小时）。
  ...
  const waitFor = (teamId, options = {}) => {
    ...
    const configured = Number(config.teamWaitMs);
    // 兜底值与配置读的是同一处：调用方（工具层）已经按配置夹过一次，这里只是不让自己裸奔。
    const fallback = Number.isSafeInteger(configured) && configured > 0 ? configured : 60 * 60 * 1000;
    const timeoutMs = Number.isSafeInteger(raw) && raw > 0 ? raw : fallback;
