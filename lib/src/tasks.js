...
export function createTaskBoard({ ctx, store, runtime, config, log }) {
  ...
  const createPlan = (input, meta = {}) => {
    ...
    // 团队模式**固定**启用任务链：这条链是群聊讨论的落地产物，群主建完链就去收尾了，
    // 没有「等人来点一下」的那个动作。所以这里不看清单级 autoActivate，也不看全局默认值。
    const teamForced = typeof store.isTeamMode === 'function' && store.isTeamMode() === true;
    const autoActivate = teamForced
      ? true
      : (input.autoActivate === undefined ? config.taskAutoActivate !== false : checked.plan.autoActivate);
    store.insertPlan({
      id,
    ...
    log.info(`新建任务清单「${checked.plan.title}」（${id}）：${checked.plan.tasks.length} 项任务，`
      + `${autoActivate ? '依赖完成自动激活' : '手动激活'}${teamForced ? '（团队模式：固定自动激活）' : ''}`);
  ...
  const setAutoActivate = (planId, value) => {
    ...
    // 团队模式开着的时候「关掉自动激活」是一个说不通的请求：团队的分工必须真的跑起来。
    // 与其静默忽略（用户会以为设置生效了），不如明确拒绝并告诉他怎么才能用手动模式。
    if (value !== true && typeof store.isTeamMode === 'function' && store.isTeamMode() === true) {
      return {
        ok: false,
        error: '团队模式开着：任务链固定自动激活。要手动放行请先在输入栏把「团队模式」关掉。',
        board: board(planId),
        started: [],
      };
    }
