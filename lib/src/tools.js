...
export function buildTools(context) {
  ...
  const teamProtocolLines = () => {
    ...
    return [
      ...
      '开完之后**讨论与执行都由宿主自己推进**：群主先发言 → 点名 → 成员发言 → 群主回话，',
      '群主会用 `team_task` 把分工派成任务链（固定自动激活），成员执行完自己用 `team_say` 汇报，',
      '任务结算时宿主也会把通知发进群并把群主叫回来。同一时刻只有一个人在说。',
      '- **这不占你的回合**：不要替团队成员传话，也不要为了等它跑完而空转；用户问进展就用 `team_status` 查。',
      '- 需要用户拍板时，群主会在群里 @用户；面板的「群组」栏会标出来。',
  ...
  const teamOpenTool = buildTool({
    name: 'team_open',
    description: [
      '按用户的团队声明开一场**群聊讨论**（团队模式）。',
      '用户消息里出现「团队名称：…」「团队负责人：@…」「团队成员：@…」三行时调用它，',
      'declaration 传那三行**原文**，mission 传用户这次要团队做的事（用户的原话，别改写）。',
      '调用后宿主会自主推进讨论：群主先发言，之后按「群主点名 → 成员发言 → 群主回话」轮流走，',
      '一次只有一个人说。**它不占你的回合**——你不需要替团队成员传话，也不需要等它跑完。',
      '群主会在收尾前把分工固化成任务链（团队模式下任务链固定自动激活）。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
    ...
    execute: async (args, exec) => {
      const { sessionId, agent: parentAgent } = callerOf(exec);
      const empty = { team_id: '', name: '', owner: '', members: [], status: '', started: false, error: '', note: '' };
      ...
      return {
        ...
        started: (opened?.status ?? '') === 'discussing',
        error: result.startError ?? '',
        note: '群里已经开聊了。你可以用 team_status 看进展；不要替团队成员传话，也不要等它跑完再回答用户。',
      };
    },
  });
  ...
  /** `team_status`：只读地看群聊与团队现状。 */
  const teamStatusTool = buildTool({
    name: 'team_status',
    description: [
      '看团队（群聊）的现状：给了 team_id 就返回群聊记录与成员状态，不给就列出所有团队。',
      '只读，不会推动讨论，也不会替谁发言。想推进讨论请让群主发言或用面板的「继续」。',
      '主对话用它了解群里谈出了什么；团队成员也可以用它回顾上下文。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
  ...
  const install = () => {
    ...
    if (toolsService !== undefined && typeof toolsService.register === 'function') {
      ...
      if (team !== undefined) toolset.push(teamOpenTool, teamSayTool, teamTaskTool, teamStatusTool);
...
/**
 * 团队工具结果的渲染（四个工具共用一段风格：结论在前、可执行信息在后）。
 * @param {object} value - `team_open` 的返回值。
 * @returns {string}
 */
function renderTeamOpen(value) {
  ...
  return [
    `团队「${value.name}」已就绪（${value.team_id}）｜状态 ${value.status}`,
    `群主：@${value.owner}`,
    `成员：${value.members.map((name) => `@${name}`).join(' ') || '（无）'}`,
    value.error === '' ? '' : `注意：${value.error}`,
    value.note,
  ].filter((line) => line !== '').join('\n');
}
