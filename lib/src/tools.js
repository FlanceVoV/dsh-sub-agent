...
export function buildTools(context) {
  ...
  if (team === undefined) {
    warnings.push('团队服务不可用：team_open / team_say / team_task / team_status 不注册（团队模式在面板上也不可用）');
  }
  ...
  /**
   * 团队模式那一段（团队服务缺席时返回空数组）。
   *
   * 为什么开关状态要写进提示词：主对话是「用户声明团队」这件事的第一读者。
   * 开关关着的时候如果不说清楚，它会一本正经地调 team_open，然后拿到一个拒绝——
   * 再然后它可能自己编一个团队继续往下演，那比直接说「让用户打开开关」糟得多。
   * @returns {string[]}
   */
  const teamProtocolLines = () => {
    if (team === undefined) return [];
    const on = team.isTeamMode() === true;
    let active = [];
    if (typeof team.pendingLines === 'function') {
      try {
        active = team.pendingLines(5);
      } catch (error) {
        log.debug(`团队 pendingLines 失败：${describeError(error)}`);
      }
    }
    return [
      '',
      '### 团队模式（群组讨论 → 派活执行）：用户声明团队时用 `team_open`',
      '开关在输入栏那一行的「团队模式」下拉里（与「子 agent 已启用」并排）。',
      on
        ? '现在是**开着**的：团队模式下任务链**固定自动激活**，没有「等我来放行」这一步。'
        : '现在是**关着**的：用户声明了团队也开不起来——先让用户把那个下拉切到「开」，再重试；不要自己编一个团队往下演。',
      '用户在消息里写下面三行，就是在**声明一个团队**：',
      '```',
      '团队名称：xxx项目组',
      '团队负责人：@xxxxx',
      '团队成员：@xxx@xxx@xxx',
      '```',
      '看到就调 `team_open`（`declaration` 传这三行**原文**，`mission` 传用户这次要团队做的事）。',
      '`team_open` **默认会一直等到群主收尾**，然后把群主的结论放进返回值（`conclusion`）交给你——',
      '所以你的回合会一直留在这一句工具调用上，**这就是「主对话不结束」的机制**，不要中途放弃。',
      '',
      '**拿到结论之后要做的**：把结论里的方案、分工、风险讲给用户听（可以精炼，但要点别丢），',
      '而不是只说一句「群聊已经开始了」。`settled` 为 false 时说明还没谈完：**不要结束回合**，',
      '把进展告诉用户，然后用 `team_status {team_id, wait:true}` 接着等（可以反复等）。',
      '',
      '另外两条要知道的：',
      '- 群主收尾时还会用 `team_report` 把结论**直接送进这个对话**——你可能已经先看到它了，别重复讲两遍。',
      '- 讨论还在跑的时候**不要替团队成员传话**，也不要自己编结论；要谈就等宿主把话题交给他们。',
      '- 需要用户拍板时，群主会在群里 @用户并停下等你；这时 `team_open` 会返回、`status` 是 `awaiting_user`，',
      '  把问题转给用户，拿到答复后用 `team_say {team_id, text, role:"user"}` 带进群，讨论就接着走。',
      '- 只有用户**明确说「不用等」**时才传 `wait:false`（他要先去做别的）；那种情况下稍后用',
      '  `team_status {team_id, wait:true}` 把结论补收回来——别让它烂在群里。',
      '- 团队模式下 `subagent_plan` 建的清单也会被强制自动激活（这是团队模式的固定条款）。',
   ...
   * 认出「现在说话的是谁」。
   *
   * 子 agent 调团队工具时，`exec.agent.session.id` 是它自己的子会话；
   * runs 表里那条记录是宿主写的，所以查它比让模型自报家门可靠。
   * @param {object} exec - 工具执行上下文。
   * @param {object} team - 团队。
   * @returns {string} 团队花名册里的名字；认不出时返回空串。
   */
  const teamSpeakerOf = (exec, team) => {
    const { sessionId } = callerOf(exec);
    if (typeof store.runBySession !== 'function') return '';
    const run = store.runBySession(sessionId);
    const name = String(run?.agentName ?? '').trim();
    if (name === team.ownerName || team.members.includes(name)) return name;
    return '';
  };

  /** 团队工具共用的「服务/开关是否就绪」检查。 */
  const teamReady = () => {
    if (team === undefined) return '团队服务不可用：本次启动没有装配它（面板的「群组」栏也看不到）';
    if (team.isTeamMode() !== true) {
      return '团队模式没打开：请让用户把输入栏那一行的「团队模式」下拉切到「开」，再重试。';
    }
    return '';
  };

  /** `team_open`：把用户声明的一个团队真正开起来。 */
  const teamOpenTool = buildTool({
    name: 'team_open',
    description: [
      '按用户的团队声明开一场**群聊讨论**（团队模式），并**等它讨论出结论再回来**。',
      '用户消息里出现「团队名称：…」「团队负责人：@…」「团队成员：@…」三行时调用它，',
      'declaration 传那三行**原文**，mission 传用户这次要团队做的事（用户的原话，别改写）。',
      '调用期间宿主会自主推进讨论：群主先发言，之后按「群主点名 → 成员发言 → 群主回话」轮流走，一次只有一个人说；',
      '**你的回合会一直等到群主收尾**，然后它把结论放进这个工具的返回值（`conclusion`）交给你——',
      '所以记得把结论讲给用户，而不是只说一句「群聊已开始」。',
      '如果返回时 `settled` 是 false，说明还没谈完：不要结束回合，用 `team_status {team_id, wait:true}` 接着等。',
      '等待上限由用户配置的 `teamWaitMs`（默认 1 小时）决定，`timeout_ms` 只能往短里调。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        declaration: { type: 'string', description: '用户原文里的团队声明（含团队名称/团队负责人/团队成员三行）' },
        mission: { type: 'string', description: '这次要团队完成的事：用户的原话或你的准确复述' },
        auto_start: { type: 'boolean', description: '是否立刻开始讨论（默认 true）。false 只建团队不开跑' },
        wait: { type: 'boolean', description: '是否等讨论收尾再返回（默认 true）。只有用户明确说「不用等」时才传 false' },
        timeout_ms: { type: 'integer', description: '等待上限（毫秒）。默认取配置 teamWaitMs（默认 1 小时）；只能在配置值之内调短，想更长请让用户改配置' },
      },
      ...
      properties: {
        ...
        messages: {
          ...
          items: {
            type: 'object',
            additionalProperties: false,
            properties: { seq: { type: 'integer' }, who: { type: 'string' }, text: { type: 'string' } },
          },
        },
        error: { type: 'string' },
        note: { type: 'string' },
    ...
    render: (_args, value) => [{ type: 'text', text: renderTeamOpen(value) }],
    isConcurrencySafe: () => false,
    // `exec` 必须接住：团队讨论要一个活着的父 agent，而工具身份是唯一可靠的来源。
    // 漏掉它的话 parentSessionId 会存成空串，讨论在第一次发言时就以
    // 「找不到会话  对应的活动 agent」中断——真机上正是这样暴露的。
    execute: async (args, exec) => {
      const { sessionId, agent: parentAgent } = callerOf(exec);
      const empty = {
        team_id: '', name: '', owner: '', members: [], status: '', started: false,
        settled: false, conclusion: '', conclusion_from: '', plan_id: '', tasks: [], messages: [], error: '', note: '',
      };
      const blocked = teamReady();
      if (blocked !== '') return { ...empty, error: blocked };
      const declaration = String(args?.declaration ?? '');
      if (declaration.trim() === '') {
        return {
          ...empty,
          error: 'declaration 必填：把用户那三行原文传进来',
          note: '需要「团队名称：…」「团队负责人：@…」「团队成员：@…」。名字必须是已配置的子 agent。',
        };
      }
      const result = team.open({
        declaration,
        mission: String(args?.mission ?? ''),
        parentSessionId: sessionId,
        parentAgent,
        autoStart: args?.auto_start !== false,
        invokedBy: 'tool',
      });
      if (!result.ok) {
        return {
          ...
          note: '把上面的原因如实告诉用户（例如某人没配置、或团队模式没打开），不要自己编一个团队。',
        };
      }
      const opened = result.team ?? team.get('');
      const base = {
        team_id: opened?.id ?? '',
        name: opened?.name ?? '',
        owner: opened?.ownerName ?? '',
        members: opened?.members ?? [],
      };
      if (result.startError !== '' && result.startError !== undefined) {
        return { ...empty, ...base, status: opened?.status ?? '', error: result.startError };
      }

      // 默认**等讨论收尾**再把结论带回去。
      // 这是「主对话不能结束」的落点：工具不返回，主对话就还在自己的回合里，
      // 而群主的结论会作为这个工具的返回值回到它手上——不需要任何额外的推送通道。
      const wantWait = args?.wait !== false;
      const timeoutMs = clampWait(args?.timeout_ms, config.teamWaitMs);
      // 等到了（或本来就已经停了）才有结论可言；`settled` 的判定只有 team 看板一处。
      const settledTeam = wantWait
        ? await team.waitFor(base.team_id, { timeoutMs })
        : (team.isSettled(base.team_id) ? team.get(base.team_id) : undefined);
      const settled = settledTeam !== undefined;
      if (settled) team.markDelivered(base.team_id);
      const packed = team.report(base.team_id, { settled, timeoutMs }) ?? {};
      const current = team.get(base.team_id);

      return {
        ...base,
        status: settledTeam?.status ?? current?.status ?? (wantWait ? 'running' : 'discussing'),
        started: true,
        settled,
        conclusion: packed.conclusion?.text ?? '',
        conclusion_from: packed.conclusion?.speaker ?? '',
        plan_id: packed.plan?.id ?? '',
        tasks: (packed.plan?.tasks ?? []).map((task) => `${task.id} ${task.title}（@${task.agent}，${task.state}）`),
        messages: (packed.messages ?? []).map((message) => ({
          seq: message.seq,
          who: message.role === 'user' ? '用户' : message.role === 'system' ? '（系统）' : `@${message.speaker}`,
          text: message.text,
        })),
        error: '',
        note: settled
          ? teamOpenSettledNote(settledTeam, packed)
          : `讨论还没结束（${packed.team?.status ?? 'running'}，第 ${packed.team?.rounds ?? 0}/${packed.team?.maxRounds ?? 0} 轮`
            + `${packed.team?.speaker ? `，正在说 @${packed.team.speaker}` : ''}）。`
            + '**不要结束回合**：把进展告诉用户，然后调 `team_status {team_id, wait:true}` 接着等结论。',
      };
    },
  });

  /**
   * 等到了之后给主对话的指引（说清「结论怎么用」，而不是只说「完成了」）。
   * @param {object} settledTeam - 团队记录。
   * @param {object} packed - `team.report()` 的产物。
   * @returns {string}
   */
  const teamOpenSettledNote = (settledTeam, packed) => {
    if (settledTeam.status === 'awaiting_user') {
      return '讨论停在**等你拍板**：把群主的问题原样转给用户，拿到答复后用 `team_say {role:"user"}` 带进群，讨论就接着走。';
    }
    if (settledTeam.status === 'error') {
      return `讨论中断了：${settledTeam.lastError || '原因见群聊记录'}。把原因如实告诉用户，需要的话修好再点「继续」。`;
    }
    if (settledTeam.status === 'paused') {
      return '讨论已暂停（可能是某个成员正忙、或用户关了团队模式）。把进展告诉用户；要继续就点面板的「继续」或调 `team_status {wait:true}`。';
    }
    return [
      '讨论已收尾，上面的 `conclusion` 就是**群主的结论/方案**，请把它讲给用户（要点、分工、风险别丢，但不要整段照抄）。',
      packed.plan === undefined
        ? '这次没有建立任务链：如果结论里有要执行的事，由你（或让用户决定）建清单或用 `@名字` 派给子 agent。'
        : `群主已经建了任务链 ${packed.plan.id}（进度 ${packed.plan.progress.done}/${packed.plan.progress.total}，团队模式下自动激活），还在跑的任务完成后会回到群里。`,
    ].join('');
  };
   ...
   * @param {number} fallback - 配置里的默认值。
   * @returns {number} 毫秒。
   */
  /**
   * 有界地解析「等多久」。
   *
   * 上限就是**用户配的那个值**（`teamWaitMs`，默认 1 小时），不是写死的常数：
   *  - 不传 → 用配置值；
   *  - 传了 → 只能在配置值之内**调短**。
   * 这条不对称是故意的：等待会占住主对话的整个回合，它的上限该由用户定，不由模型定。
   * 想等更久，用户改配置；模型可以用 `team_status {wait:true}` 反复续等，不必突破上限。
   *
   * @param {unknown} raw - 工具参数（毫秒）。
   * @param {number} configured - 配置里的 `teamWaitMs`。
   * @returns {number} 毫秒。
   */
  const clampWait = (raw, configured) => {
    const ceiling = Number.isSafeInteger(configured) && configured > 0 ? configured : 60 * 60 * 1000;
    const value = Number(raw);
    if (!Number.isFinite(value) || value <= 0) return ceiling;
    return Math.min(Math.max(Math.round(value), 1000), ceiling);
  };

  /** `team_say`：成员在群里发言（汇报、求助、争论）。 */
  const teamSayTool = buildTool({
    name: 'team_say',
    description: [
      '在所属团队的群聊里说一句（团队模式的通信口）。',
      '你是团队**成员**时：任务做完要立刻用它向群主汇报（一句话结论 + 产物位置）；',
      '碰到任何阻塞、歧义、风险或需要别人配合，也立刻用它 @群主 请求处理——不要自己猜着往下做。',
      '要求：短（一个问题 ≤ 群规字数），结论先行，只讲别人不知道的东西。',
      '说完之后话语权会自动回到群主，你不需要点名别人。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        team_id: { type: 'string', description: '团队 id（任务说明里给的「团队群 id」）' },
        text: { type: 'string', description: '你要说的话。简洁、言之有物；需要群主处理就 @群主' },
        role: { type: 'string', enum: ['auto', 'user'], description: 'auto（默认）= 以你自己的身份发言；user = 代用户把话带进群（会重启讨论）' },
      },
    ...
    render: (_args, value) => [{ type: 'text', text: renderTeamSay(value) }],
    isConcurrencySafe: () => true,
    execute: async (args, exec) => {
      const empty = { ok: false, seq: 0, team_id: '', speaker: '', role: '', error: '', note: '' };
      if (team === undefined) return { ...empty, error: '团队服务不可用：本次启动没有装配它' };
      const teamId = String(args?.team_id ?? '').trim();
      const found = teamId === '' ? undefined : team.get(teamId);
      if (found === undefined) {
        const list = team.list({ limit: 20 });
        return {
          ...empty,
          error: teamId === '' ? 'team_id 必填' : `没有这个团队：${teamId}`,
          note: list.length === 0 ? '现在一个团队都没有。' : `现有团队：${list.map((item) => `${item.name}（${item.id}）`).join('、')}`,
        };
      }
      const text = String(args?.text ?? '').trim();
      if (text === '') return { ...empty, error: 'text 必填（要说的那句话）', team_id: teamId };

      // 代用户带话：显式声明，避免把模型的话算到用户头上。
      if (String(args?.role ?? '') === 'user') {
        const said = team.userSay(teamId, text);
        return {
          ...empty,
          ok: said.ok === true,
          seq: said.message?.seq ?? 0,
          team_id: teamId,
          speaker: '用户',
          role: 'user',
          error: said.error ?? '',
          note: said.ok ? '已经把用户的话带进群，讨论会接着走。' : '',
        };
      }

      const speaker = teamSpeakerOf(exec, found);
      if (speaker === '') {
        return {
          ...empty,
          team_id: teamId,
          error: `认不出你是谁：「${found.name}」的成员是 ${[found.ownerName, ...found.members].map((name) => `@${name}`).join(' ')}`,
          note: 'team_say 要在团队成员的会话里调用（成员从任务说明里拿到 team_id）。'
            + '如果你要把**用户**的话带进群，用 role:"user"。',
        };
      }
      const said = team.say({
        teamId,
        role: speaker === found.ownerName ? 'owner' : 'member',
        speaker,
        text,
      });
      if (!said.ok) return { ...empty, team_id: teamId, error: said.error ?? '发言失败' };
      return {
        ok: true,
        seq: said.message?.seq ?? 0,
        team_id: teamId,
        ...
        role: speaker === found.ownerName ? 'owner' : 'member',
        error: '',
        note: speaker === found.ownerName ? '' : `已进群，群主 @${found.ownerName} 会被叫醒处理。`,
      };
    },
  });

  /** `team_task`：群主把分工固化成一条任务链。 */
  const teamTaskTool = buildTool({
    name: 'team_task',
    description: [
      '（群主专用）**派活**：把讨论定的分工变成任务链，并立刻开跑。',
      '可以反复调用：第一次建链，之后每次都是往同一条链上续派（追加任务）。',
      '团队模式下任务链固定自动激活：没有依赖的任务马上开始，有依赖的等上游完成；',
      '任务跑完或失败时，宿主会把汇报发到群里并叫你回来处理——所以派完可以用 `@等待` 挂起讨论。',
      '每个任务写清楚：谁做（agent，必须是团队成员）、做什么（brief，完整到别人不用问你）、依赖谁（deps）。',
      '讨论还没收敛、或还不需要动手时，不要调用它。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        ...
        title: { type: 'string', description: '这条链的名字（默认「团队 X 的任务链」）' },
        note: { type: 'string', description: '给这条链的备注（比如验收标准）' },
        tasks: {
          type: 'array',
          description: '任务清单。id 可省（宿主按 t1..tN 编号），deps 引用同一批里的 id',
          items: {
            type: 'object',
            additionalProperties: false,
            properties: {
              id: { type: 'string', description: '任务 id（可省）' },
              title: { type: 'string', description: '一句话任务名' },
              agent: { type: 'string', description: '执行人（必须是团队成员的名字）' },
              brief: { type: 'string', description: '完整任务说明' },
    ...
    render: (_args, value) => [{ type: 'text', text: renderTeamTask(value) }],
    isConcurrencySafe: () => false,
    execute: async (args, exec) => {
      const empty = { ok: false, plan_id: '', title: '', tasks: [], started: [], error: '', note: '' };
      const blocked = teamReady();
      if (blocked !== '') return { ...empty, error: blocked };
      const teamId = String(args?.team_id ?? '').trim();
      const found = teamId === '' ? undefined : team.get(teamId);
      ...
      const speaker = teamSpeakerOf(exec, found);
      if (speaker !== '' && speaker !== found.ownerName) {
        return {
          ...empty,
          error: `只有群主 @${found.ownerName} 能建任务链，你现在是 @${speaker}`,
          note: '把分工交给群主去定；如果你对方案有意见，用 team_say 说出来。',
        };
      }
      const result = team.planTasks(teamId, {
        title: args?.title,
        note: args?.note,
        tasks: Array.isArray(args?.tasks) ? args.tasks : [],
      }, { invokedBy: 'tool' });
      if (!result.ok) return { ...empty, error: (result.errors ?? []).join('；') };
      ...
      return {
        ...
        tasks: (board.tasks ?? []).map((task) => ({
          id: task.id, title: task.title, agent: task.agentName, state: task.state, deps: task.deps,
        })),
        started: (result.started ?? []).map((item) => item.taskId ?? item.task_id ?? ''),
        error: '',
        note: speaker === '' ? '（这次是主对话代为建链的，群里已经注明。）' : '链已经开跑了，成员会各自用 team_say 汇报。',
      };
    },
  });

  /** `team_status`：看群聊与团队现状（可选等它收尾）。 */
  const teamStatusTool = buildTool({
    name: 'team_status',
    description: [
      '看团队（群聊）的现状：给了 team_id 就返回群聊记录与成员状态，不给就列出所有团队。',
      '`wait:true` 时会**等到这场讨论收尾**再把群主的结论一起带回来——',
      '`team_open` 超时返回（settled=false）之后，就用它接着等，不要结束回合。',
      '等待上限取配置的 `teamWaitMs`（默认 1 小时），`wait_ms` 只能往短里调。',
      '默认只读、不推动讨论、不替谁发言；团队成员也可以用它回顾上下文。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        team_id: { type: 'string', description: '团队 id；省略则列出所有团队' },
        since: { type: 'integer', description: '只要 seq 大于它的新消息（增量看）' },
        limit: { type: 'integer', description: '最多返回多少条消息（默认 30）' },
        wait: { type: 'boolean', description: '是否等这场讨论收尾再返回（默认 false）。配 wait_ms 控制上限' },
        wait_ms: { type: 'integer', description: '等待上限（毫秒）。默认取配置 teamWaitMs（默认 1 小时）；只能在配置值之内调短' },
      },
      required: [],
    },
    outputSchema: {
    ...
    render: (_args, value) => [{ type: 'text', text: renderTeamStatus(value) }],
    isConcurrencySafe: () => true,
    execute: async (args) => {
      if (team === undefined) return { teams: [], error: '团队服务不可用：本次启动没有装配它' };
      const teamId = String(args?.team_id ?? '').trim();
      if (teamId === '') {
        const summary = team.summary();
        return { teams: summary.teams, error: '' };
      }
      // 等它收尾（可选）。等到了就把结论一并交出去，并标记「已带回主对话」。
      if (args?.wait === true) {
        const timeoutMs = clampWait(args?.wait_ms, config.teamWaitMs);
        const settledTeam = await team.waitFor(teamId, { timeoutMs });
        if (settledTeam !== undefined) team.markDelivered(teamId);
        const packed = team.report(teamId, { settled: settledTeam !== undefined, timeoutMs });
        if (packed === undefined) return { teams: [], error: `没有这个团队：${teamId}` };
        const view = team.stateOf(teamId, { limit: 30 });
        return {
          teams: [],
          team_id: packed.team.id,
          name: packed.team.name,
          status: packed.team.status,
          phase: packed.team.phase,
          speaker: packed.team.speaker,
          rounds: packed.team.rounds,
          max_rounds: packed.team.maxRounds,
          owner: packed.team.ownerName,
          members: [packed.team.ownerName, ...packed.team.members],
          plan_id: packed.team.planId,
          plan: packed.plan ?? {},
          conclusion: packed.conclusion?.text ?? '',
          conclusion_from: packed.conclusion?.speaker ?? '',
          settled: settledTeam !== undefined,
          messages: (view?.messages ?? []).map((message) => ({
            seq: message.seq, role: message.role, speaker: message.speaker, text: message.text, at: message.createdAt,
          })),
          note: settledTeam === undefined
            ? '还没等到收尾（到等待上限了）。**不要结束回合**：把进展告诉用户，再调一次 `team_status {team_id, wait:true}`。'
            : teamOpenSettledNote(settledTeam, packed),
          error: '',
        };
      }
      const since = Number.isSafeInteger(args?.since) ? Number(args.since) : 0;
      const limit = Number.isSafeInteger(args?.limit) && args.limit > 0 ? Number(args.limit) : 30;
      const view = team.stateOf(teamId, since > 0 ? { since, limit } : { limit });
      if (view === undefined) return { teams: [], error: `没有这个团队：${teamId}` };
      const latest = view.messages.at(-1)?.seq ?? 0;
      // 这一次把结论交出去了就算「已转达」，提示词里不再重复提醒。
      const conclusion = view.team.status === 'closed' ? team.conclusionOf(teamId) : undefined;
      if (conclusion !== undefined) team.markDelivered(teamId);
      return {
        teams: [],
        team_id: view.team.id,
        name: view.team.name,
        status: view.team.status,
        phase: view.team.phase,
        speaker: view.team.speaker,
        rounds: view.team.rounds,
        max_rounds: view.team.maxRounds,
        owner: view.team.ownerName,
        members: [view.team.ownerName, ...view.team.members],
        plan_id: view.team.planId,
        plan: view.plan ?? {},
        // 已经收尾的：顺手把群主结论带上（这样 `team_status` 单独一次调用也能「取走结论」）。
        conclusion: conclusion?.text ?? '',
        conclusion_from: conclusion?.speaker ?? '',
        settled: team.isSettled(teamId),
        note: view.team.status === 'awaiting_user'
          ? '群主在等用户拍板：把问题转给用户，拿到答复后用 `team_say {role:"user"}` 带进群。'
          : '',
        messages: view.messages.map((message) => ({
          seq: message.seq,
          role: message.role,
  ...
  /**
   * `team_report`：群主把结论**直接投回主对话**。
   *
   * 为什么需要它：`team_open` 会挡在主对话的回合里等结论，那条是主通道；
   * 但只要主对话的回合提前结束了（超时、或模型自己收尾），结论就得有个办法**主动找上门**。
   * DSH 的 `ctx.subagents.sendMessage(sender, targetId, ...)` 正是这个口子：
   * 「把消息投给发送者的直接父会话；对方空闲时会**起一个新回合**」。
   *
   * 它的硬约束是**发送者必须是一个活着的直接子 agent**。群主在自己的那一轮里调这个工具时，
   * `exec.agent` 恰好就是那个活着的子 agent（团队运行是以主对话为父发布出去的），
   * 所以这是唯一能成立的位置——宿主自己拿不到这样一个身份。
   *
   * 失败绝不静默：投不出去就如实说明，并告诉群主/主对话「结论仍在群里，用 team_status 取」。
   */
  const teamReportTool = buildTool({
    name: 'team_report',
    description: [
      '（群主专用）把**结论/方案送进主对话**：主对话会因此收到一条消息，必要时被唤醒继续干活。',
      '收尾（`@收尾`）之前调一次，text 传你的结论本身（目标、方案、分工、风险、验收），而不是「我总结完了」。',
      '不传 text 时自动取你在群里的最后一段发言。',
      '主对话在等这场讨论时（它调过 team_open 且还没返回），这句话会与它的返回值汇合，不会丢。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        team_id: { type: 'string', description: '团队 id' },
        text: { type: 'string', description: '要带回主对话的结论正文；省略则取你最后一段群聊发言' },
      },
      required: ['team_id'],
    },
    outputSchema: {
    ...
    render: (_args, value) => [{ type: 'text', text: renderTeamReport(value) }],
    isConcurrencySafe: () => false,
    execute: async (args, exec) => {
      const empty = { ok: false, delivered: false, team_id: '', to: '', error: '', note: '' };
      if (team === undefined) return { ...empty, error: '团队服务不可用：本次启动没有装配它' };
      const teamId = String(args?.team_id ?? '').trim();
      const found = teamId === '' ? undefined : team.get(teamId);
      if (found === undefined) return { ...empty, error: teamId === '' ? 'team_id 必填' : `没有这个团队：${teamId}` };

      const speaker = teamSpeakerOf(exec, found);
      if (speaker !== found.ownerName) {
        return {
          ...empty,
          team_id: teamId,
          error: `只有群主 @${found.ownerName} 能把结论带回主对话，你现在是 ${speaker === '' ? '（认不出来）' : `@${speaker}`}`,
          note: '把结论交给群主去送；你如果发现结论有问题，用 team_say 说出来。',
        };
      }
      const text = String(args?.text ?? '').trim() || (team.conclusionOf(teamId)?.text ?? '');
      if (text === '') {
        return { ...empty, team_id: teamId, error: '没有可送的结论：请传 text，或在群里先说一段结论' };
      }
      const target = String(found.parentSessionId ?? '');
      if (target === '') {
        return {
          ...empty,
          team_id: teamId,
          error: '这个团队没有父会话（可能是旧版本建的）：结论送不回去',
          note: '在面板的「群组」里点一次「继续」会把当前会话补记上，之后就能送了；结论此刻仍在群里。',
        };
      }
      const sender = exec?.agent;
      if (sender === undefined || sender === null) {
        return { ...empty, team_id: teamId, to: target, error: '拿不到发送者身份：这个工具必须由群主在自己的那一轮里调用' };
      }
      if (subagentsService === undefined || typeof subagentsService.sendMessage !== 'function') {
        // 「这个部署根本没有这条通道」也要让用户看见：否则他只会看到主对话没收到结论。
        try {
          team.announce(teamId, '（系统）结论没能直接投进主对话：这个部署的 DSH 没有 `subagents.sendMessage`。'
            + '它会由主对话收尾时的 `team_open` 返回值带回（结论本身没有丢）。');
        } catch (error) {
          log.debug(`团队公告失败：${describeError(error)}`);
        }
        return {
          ...empty,
          team_id: teamId,
          to: target,
          error: '本部署的 DSH 没有 subagents.sendMessage：结论无法直投主对话',
          note: '结论仍然在群聊记录里（以及团队收尾时主对话的 team_open 返回值里），没有丢。',
        };
      }
      const controller = new AbortController();
      /** 失败时往群里发一条公告：失败原因只有模型看得到，而用户在看的**是面板**。 */
      const announce = (message) => {
        try {
          team.announce(teamId, message);
        } catch (error) {
          log.debug(`团队公告失败：${describeError(error)}`);
        }
      };
      try {
        const messageId = await withTimeout(
          subagentsService.sendMessage(sender, target, [{ type: 'text', text: `【团队结论｜${found.name}｜群主 @${found.ownerName}】\n${text}` }], {
            signal: controller.signal,
          }),
          15_000,
        );
        if (messageId === undefined) {
          controller.abort(new Error('team_report timeout'));
          announce('（系统）结论没能直接投进主对话：投递超时。结论仍在群里，主对话收尾时也会从 `team_open` 的返回值拿到它。');
          return {
            ...empty,
            team_id: teamId,
            to: target,
            error: '投递超时（15s）：主对话可能正忙，或这条通道在这个部署里不可用',
            note: '结论仍在群聊记录里；主对话也会从 team_open 的返回值拿到它。',
          };
        }
        return { ok: true, delivered: true, team_id: teamId, to: target, error: '', note: `已投给主对话（${String(messageId)}）。` };
      } catch (error) {
        announce(`（系统）结论没能直接投进主对话：${describeError(error)}。结论仍在群里，主对话收尾时也会从 \`team_open\` 的返回值拿到它。`);
        return {
          ...empty,
          team_id: teamId,
          to: target,
          error: `投递失败：${describeError(error)}`,
          note: '常见原因：这个部署的 provider 不支持向父会话投递、父会话已经不在了。'
            + '结论没有丢——它仍在群聊记录里，主对话收尾时也会从 team_open 的返回值拿到它。',
        };
      }
    },
  });
  ...
  const install = () => {
    ...
    if (toolsService !== undefined && typeof toolsService.register === 'function') {
      ...
      if (team !== undefined) toolset.push(teamOpenTool, teamSayTool, teamTaskTool, teamStatusTool, teamReportTool);
    ...
    log.debug(`model-facing layer installed: tools=${toolsService !== undefined} prompt=${promptService !== undefined} tasks=${tasks !== undefined} team=${team !== undefined}`);
    return () => {
 ...
 * `team_open` 的渲染。
 *
 * 形状说明：**结论在最前面**。主对话读这个返回值时的第一件事就是「群主谈出了什么」，
 * 把它压在一堆元信息后面，模型很容易只复述「已就绪」——那正是真机上发生过的事。
 *
 * @param {object} value - 返回值。
 * @returns {string}
 */
function renderTeamOpen(value) {
  if (value.error !== '' && value.team_id === '') {
    return `团队没能开起来：${value.error}${value.note === '' ? '' : `\n${value.note}`}`;
  }
  const lines = [
    `团队「${value.name}」｜状态 ${value.status}｜${value.settled ? '已收尾' : '**还没结束**'}`,
    `群主：@${value.owner}　成员：${value.members.map((name) => `@${name}`).join(' ') || '（无）'}`,
  ];
  if (value.error !== '') lines.push(`注意：${value.error}`);
  if (typeof value.conclusion === 'string' && value.conclusion !== '') {
    lines.push('', `群主 @${value.conclusion_from || value.owner} 的结论：`, value.conclusion);
  }
  if (Array.isArray(value.tasks) && value.tasks.length > 0) {
    lines.push('', `任务链 ${value.plan_id}：`, ...value.tasks.map((task) => `- ${task}`));
  }
  if (Array.isArray(value.messages) && value.messages.length > 0) {
    lines.push('', '群聊最近几条：', ...value.messages.map((message) => `#${message.seq} ${message.who}：${message.text}`));
  }
 ...
 * @returns {string}
 */
function renderTeamReport(value) {
  if (value.ok !== true) {
    return `结论没送到主对话：${value.error}${value.note === '' ? '' : `\n${value.note}`}`;
  }
  return `结论已投给主对话（会话 ${value.to}）：主对话会收到一条消息，必要时被唤醒继续往下做。`;
}
 ...
 * @returns {string}
 */
function renderTeamSay(value) {
  if (value.ok !== true) return `发言没有送出去：${value.error}${value.note === '' ? '' : `\n${value.note}`}`;
  return `已进群（第 ${value.seq} 条，${value.role === 'owner' ? '群主' : value.role === 'user' ? '用户' : '成员'} @${value.speaker}）`
    + `${value.note === '' ? '' : `\n${value.note}`}`;
}
 ...
 * @returns {string}
 */
function renderTeamTask(value) {
  if (value.ok !== true) return `派活失败：${value.error}${value.note === '' ? '' : `\n${value.note}`}`;
  return [
    `任务链 ${value.plan_id}「${value.title}」已派发`,
    ...value.tasks.map((task) => `- ${task.id}「${task.title}」→ @${task.agent}｜${task.state}`
      + `${task.deps.length > 0 ? `｜依赖 ${task.deps.join('、')}` : ''}`),
    value.started.length === 0 ? '（此刻没有任务被立刻启动：等依赖完成，或它们已在跑）' : `立刻启动：${value.started.join('、')}`,
    value.note,
  ].filter((line) => line !== '').join('\n');
}
...
function renderTeamStatus(value) {
  if (value.error !== '' && value.team_id === '') {
    return `看不了团队：${value.error}`;
  }
  if (value.team_id === '' || value.team_id === undefined) {
    if (value.teams.length === 0) return '现在一个团队都没有（团队模式下的群聊会在这里列出来）。';
    return [
      `共 ${value.teams.length} 个团队：`,
      ...value.teams.map((team) => `- ${team.name}（${team.id}）｜群主 @${team.ownerName}｜成员 `
        + `${team.members.map((name) => `@${name}`).join(' ') || '（无）'}｜${team.status}`
        + `｜第 ${team.rounds}/${team.maxRounds} 轮｜${team.lastSeq} 条消息`
        + `${team.planId === '' ? '' : `｜任务链 ${team.planId}`}`
        + `${team.last === '' ? '' : `\n  最后一句：${team.last}`}`),
    ].join('\n');
  }
  const session = value.speaker === '' ? '' : `｜正在说：@${value.speaker}`;
  return [
    `团队「${value.name}」（${value.team_id}）｜${value.status}／${value.phase}${session}｜第 ${value.rounds}/${value.max_rounds} 轮`,
    `群主：@${value.owner}　成员：${value.members.filter((name) => name !== value.owner).map((name) => `@${name}`).join(' ') || '（无）'}`,
    value.plan_id === '' ? '任务链：（还没有）' : `任务链：${value.plan_id}｜进度 ${value.plan?.progress?.done ?? 0}/${value.plan?.progress?.total ?? 0}`,
    typeof value.conclusion === 'string' && value.conclusion !== ''
      ? `群主结论（${value.conclusion_from || value.owner}）：${value.conclusion}`
      : '',
    value.note === undefined ? '' : value.note,
    '',
    '群聊记录：',
    ...value.messages.map((message) => {
      const who = message.role === 'user' ? '用户' : message.role === 'system' ? '（系统）' : `@${message.speaker}`;
      return `#${message.seq} ${who}：${message.text}`;
    }),
  ].filter((line) => line !== '').join('\n');
}
