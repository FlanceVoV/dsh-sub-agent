...
export function buildTools(context) {
  ...
  const teamOpenTool = buildTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        timeout_ms: { type: 'integer', description: '等待上限（毫秒），默认取插件配置的 teamWaitMs（15 分钟），上限 1 小时' },
      },
  ...
  const clampWait = (raw, fallback) => {
    const fallbackMs = Number.isSafeInteger(fallback) && fallback > 0 ? fallback : 15 * 60 * 1000;
    const value = Number(raw);
    if (!Number.isFinite(value) || value <= 0) return fallbackMs;
    return Math.min(Math.max(Math.round(value), 1000), 60 * 60 * 1000);
  };
  ...
  const teamStatusTool = buildTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        wait_ms: { type: 'integer', description: '等待上限（毫秒），默认取插件配置的 teamWaitMs' },
      },
      required: [],
    },
    outputSchema: {
