...
export function buildTools(context) {
   ...
   * @param {{detail?:boolean}} [options] - 是否带上任务说明与产出摘要。
   * @returns {string}
   */
  ...
  const planTool = buildTool({
    ...
    description: [
      ...
      '建完用 subagent_tasks 看整条链路。往已有清单追加任务时给 plan_id。',
    ].join(' '),
    parameters: {
      type: 'object',
      additionalProperties: false,
    ...
    execute: async (args, exec) => {
      const { sessionId, agent: parentAgent } = callerOf(exec);
      const fail = (error) => ({ plan_id: '', title: '', chain: '', started: [], error });
      if (!isEnabled()) {
      ...
      const chain = [
        renderPlanView(view, { detail: false }),
        view.plan.autoActivate !== true
      ...
      return {
        ...
        chain,
        started: result.started.map((item) => ({ task_id: item.taskId, run_id: item.runId, agent: item.agentName })),
        error: '',
      };
    },
  });
  ...
  const tasksTool = buildTool({
    ...
    execute: async (args, exec) => {
      ...
      if (action === 'view') {
        ...
        const blocks = views.map((view) => renderPlanView(view, { detail }));
        const head = [];
