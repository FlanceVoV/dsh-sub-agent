...
| 这条依赖解锁了没有 | 连线的**样式**：实线=上游已完成、流动虚线=上游正在跑、灰虚线=还没轮到 |

**走向跟着容器宽度变**：悬浮球面板（348px）用**上 → 下**的纵向流水线——
横向在那个宽度里只能看到「第一个任务 + 半个第二个任务」，比不画还糟；
设置页用**左 → 右**的横向依赖图。两种走向共用同一套分层与状态语义，只是把轴换了一下。
...
| 面板 / 设置页 | 画图：谁在跑、谁完成、谁在等谁、谁被卡住；设置页还提供手动推进入口 |
...
1. **链路需要一个活着的父会话。** 自动激活要用父 agent 实例起子 agent；
   拿不到就**不启动**，并把原因写进清单（界面上能看到「为什么这条链路卡住了」），
   而不是假装启动成功。
2. **主对话不会被打断通知。** 自动激活发生在没有工具调用的时刻，
   没有任何通道能往主对话里推消息。所以「链路跑到哪了」靠主对话主动调 `subagent_tasks`——
   工具输出里也会带上链路现状（进度、下一步、被阻塞的任务）。
...
6. **删清单不删运行记录。** 清单与任务是「计划」，运行与评价是「台账」，
