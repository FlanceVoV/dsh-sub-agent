...
需要**多个角色一起判断**的事情（方案怎么定、谁在哪被卡住），可以开一场**团队模式**的群聊：
群主（负责人）主持、成员逐个发言、轮流制，讨论出的分工由群主直接派成任务链去执行，
跑完的汇报回到群里——见「团队模式（群组）」。
...
输入栏那一行的「团队模式」下拉切到**开**，然后在消息里声明团队：
...
群里立刻开聊：群主先讲判断并点名，成员逐个发言（同一时刻只有一个人说），
话语权每次都回到群主——它可以继续点名、`@等待`（等派出去的活跑完）、`@用户`（要你拍板）或 `@收尾`。
群主随时用 `team_task` 把已经谈定的分工**直接派成任务链去执行**（边谈边派，不必等讨论结束），
任务跑完由宿主把汇报发回群里并把群主叫回来。展开面板的**「群组」页签**看全过程，也可以直接插话。

细节（协议、边界、四个工具、配置项）见「团队模式（群组）」一节。
...
一旦任务需要**多个角色的判断**（谁的方案更稳、谁在哪些约束上会被卡住），
一问一答就退化成「主对话替所有人转述」——而主对话没有各方的独立上下文，
转述出来的「共识」往往是它自己的观点被复述三遍。

团队模式把这件事搬进一个**真正的群聊**：群主（负责人）主持、成员各自发言、轮流制（同一时刻只有一个人说），
讨论出的分工由群主**直接派成任务链去执行**，执行中的汇报与求助回到群里，群主据此再安排下一步。

### 怎么开

1. 输入栏那一行（与「子 agent 已启用」并排）把**「团队模式」下拉切到「开」**。
   这是全局开关：关掉它会把正在讨论的团队一起暂停（开关的语义是「别再自动说话了」，不是「藏起来」）。
2. 在你的消息里**声明团队**，就是下面三行：
...
这是这块设计里最要紧的一条：讨论不能「聊完就散」，结论必须回到主对话、并由它讲给用户。
...
2. **直投通道 —— 群主用 `team_report` 把结论主动推进主对话。**
   ...
   所以这是唯一能成立的位置。
   投不出去**绝不静默**：工具会返回原因，同时往群聊里发一条宿主公告——用户看的是面板，失败要看得见。
3. **兜底 —— 没被取走的结论会出现在提示词里。**
   已收尾、但结论从没被任何工具带走的团队（最近 12 小时），会在主对话的每一轮提示词里列出来
   （团队名 + 群主的结论），直到有人用 `team_open` / `team_status` 把它取走。
...
我（主对话）──team_open（等）──────────────┐
   ...
   └── 返回值：群主结论 / 方案 / 任务链 ──┘
   ▲
   └── team_report 直投（主对话空闲时会被唤醒）—— 失败则群里公告 + 兜底反复提醒
```

### 群里怎么走（协议）

驱动循环在**宿主**里，不在主对话里：窗口刷新不影响它，主对话的回合也不会把它掐断
（主对话此刻正挡在 `team_open` 上等它，这是有意的）。

```
群主开场（讲判断 + 点名）
   → 被点名的成员发言（一次一个）
   → 话语权回到群主
   → 群主回话 / 继续点名 / @等待（等任务）/ @用户（要你拍板）/ @收尾
```

- **群主用最后一行指挥**：`@成员`（可一行点多人，按顺序）、`@用户`、`@等待`、`@收尾`。
  正文里随口提到的 `@名字` 不会被当成调度指令（只认行首与显式标记）。
- **短**：单条发言上限 `teamMessageChars`（默认 300 字）。超了不删，但会在注入下一轮时截断并标「超长」。
- **可以争论**：群规明写「觉得站不住脚就直接反驳」「一致不是目标，正确才是」「第一性原理、独立思考」。
- **必然收敛**：群主主持到 `teamMaxRounds` 轮（默认 3，含开场）就**强制进入收尾轮**，
  无论模型还想不想接着说——这是终止保证，也是费用护栏。
- 群主漏给「下一步」时先提醒一次（`teamMaxNudges`），提醒用完就按已有内容收尾，不会无限空转。

### 讨论直接接上执行

- 群主的派活动作是 `team_task`：**第一次建链，之后每次都是往同一条链上续派**。
  所以可以边谈边派，不必等讨论结束（开场的提示词就是这么要求的）。
- **团队模式下任务链固定自动激活**（`createPlan` 里硬写 `true`，并拒绝在团队模式开着时关掉自动激活）：
  团队的分工必须真的跑起来，没有「等用户来点一下」那个动作。
- 每个任务说明里都会注入**群协议**：完成后立刻用 `team_say` 向群主汇报；
  碰到任何阻塞、歧义、风险、或需要别人配合，立刻 `team_say @群主` 请求处理，不要自己猜着往下做。
- **宿主侧还有一道保险**：任务一结算，宿主就把「@群主 任务 t2 已完成/失败」发进群里并把群主叫回来。
  提示词是可以被忽略的，而「谁跑完了」宿主是权威知情者——协作的关键动作不该押在模型自觉上。
- 群主可以用 `@等待` 挂起讨论等结果（链上确实有任务在跑时才允许，否则会被拦下来）。

### 边界（都是真的）

1. **面板不提供「下一位」按钮。** 发言调度只有一个地方——宿主循环。再加一个按钮就等于第二个调度者，
   而模型上下文里并没有「用户替我点了谁」这件事。面板只有暂停 / 继续 / 结束，和所有人都有的「插话」。
2. **一个团队同时只推进一场发言**，但派出去的任务与讨论是并行的（并发上限仍由 `maxConcurrentRuns` 管）。
3. **同一个子 agent 不能同时开两次运行**：轮到它发言时它正在跑任务，讨论会**暂停**并说明是谁在忙，
   等它跑完点「继续」即可（不是失败）。
4. **宿主重启后讨论不会自己继续**：启动对账会把「讨论中」改写成「已暂停」并说明原因，
   而不是让界面显示一场没人推动的讨论（点「继续」就接着走）。
5. **群聊记录是有界注入的**：每轮发言带上最近 `teamTranscriptChars`（默认 6000 字符）的群聊记录，
   子 agent 之间不共享上下文，这份记录就是它们的共享记忆；超出的部分明确写出省略了多少条。
...
| `taskAutoActivate` | true | 新建任务清单时「依赖完成自动激活下游」的默认值（逐条清单可覆盖；团队模式下恒为真） | ✅ |
| `taskUpstreamChars` | 2000 | 注入下游提示词的上游产出字符上限（每个依赖各算一次） | ✅ |
| `teamMaxRounds` | 3 | 团队讨论群主最多主持几轮（含开场），到点强制收尾 | ✅ |
...
团队（`teams` / `team_messages`）**存状态**，这一条刻意与任务相反：
任务的 state 能从依赖图现算，而「群主刚才点了谁发言」算不出来——
它是事实本身。代价是「进程重启后界面可能在显示一场没人推动的讨论」，
所以启动对账会把「讨论中 / 等执行」改写成「已暂停」并说明原因。

数据库带 `application_id = 0x53554241`（`'SUBA'`）：**拿错文件会直接拒绝打开**，
绝不往别人的库里写。

**升级是自动的**：老库在首次启动时会被补建缺的表并升到当前 schema 版本
（只加表，幂等），启动日志里会打一行 `数据库 schema 已升级：v1 → v3`。
`0.3.0` 补的是 `teams` / `team_messages`（schema v2 → v3）。
...
| GET | `/team` | 团队列表与团队模式开关（面板「群组」页签的数据源） |
| POST | `/team` | 从声明开一个团队（body：`{declaration, mission, parentSessionId?}`） |
| GET | `/team/:teamId?since=&limit=` | 群聊记录 + 花名册 + 它派出去的任务链；`since` 增量取 |
| POST | `/team/mode` | 团队模式开关（body：`{enabled}`；关掉会顺便暂停讨论中的团队） |
| POST | `/team/:teamId/say` | 用户亲自在群里说一句（body：`{text}`） |
| POST | `/team/:teamId/start\|pause\|close` | 继续 / 暂停 / 结束这场讨论 |
...
团队模式再添五个：`team_open`（按用户声明开一场群聊，**默认等到群主收尾并把结论带回来**）、
`team_say`（在群里发言/汇报/求助）、`team_task`（群主派活，可反复调用往同一条链上续派）、
`team_status`（看群里谈了什么；`wait:true` 时等到收尾并把结论带回）、
`team_report`（群主把结论**直接投进主对话**）。
...
当前顺序是 `01-head` → `02-orb-ball` → `03-detail-boards` → `04-task-graph` → `045-team-group` → `05-settings-tail`；
`045-team-group.js` 这个名字是为了排在 `04-` 之后、`05-` 之前（按文件名排序，`04-` < `045` < `05`）。
...
| 能力 | 关掉之后你会看到 |
| --- | --- |
| `subagents` | `@` 委派跑不起来，悬浮球永远没有任务（团队讨论也会在「起发言」这一步停住，群里会写明原因） |
| `tools` | 主对话**看不到** `subagent_run` 等那批工具（面板仍能用） |
| `systemPrompt` | `@` 的注入协议段没进系统提示词，主对话不知道该怎么处理 `@`，也不知道团队声明协议 |
| `credentials` | 配置页的凭据引用拿不到状态 |
| `llm` / `settings` / `sessions` | 模型清单、设置读写、会话查询相应降级 |
| `team`（`/health` 里单列） | 团队模式整块不可用：面板的「群组」页签显示空状态，`/team` 系列端点回 503 |
...
**「群组」页签里没有团队 / 讨论不动？** 按这个顺序看：

1. **团队模式是不是关着**：它有自己的全局开关（输入栏那一行）。关着的时候用户声明了团队也开不起来，
   主对话拿到的拒绝理由里就写着这一条；页签里的空状态也会直接把声明模板摆出来。
2. **`/health` 的 `team.available` 是不是 `false`**：那说明团队服务没装配
   （启动日志里有一条 `团队模式未启用（其余能力不受影响）`），通常是存储没起来——
   先看 `/health` 的 `initError`。
3. **状态是「已暂停」**：这是**如实**的，不是 bug——可能是宿主重启过（启动对账会把
   「讨论中 / 等执行」改成「已暂停」），也可能是某个成员正忙（群里会写明是谁），
   或者团队模式被关掉时顺手暂停了。点「继续」就接着走。
4. **状态是「出错了」**：群聊最后一条会写明原因（例如「找不到团队的父会话」——
   团队运行需要一个活着的父会话，和任务链同理）。
...
lib/src/store.js       sqlite：agents / runs / rounds / evaluations / settings / plans / tasks / teams / team_messages
...
lib/src/team.js        团队模式：声明/指令解析、群聊驱动循环、群主派活（唯一碰「谁在说话」处）
lib/src/runtime.js     运行时：起子 agent、折遥测、守并发与超时
lib/src/tools.js       模型侧入口：九个工具 + 系统提示词协议段
lib/src/http.js        JSON + SSE 接口层
lib/src/version.js     DSH 版本探测与兼容性判定（零 DSH 依赖，只读 package.json）
lib/parts/*.js         客户端分片（唯一真相来源；01 head → 045 群组 → 05 收尾）
...
- **0.3.0**：团队模式（群组讨论 + 群主派活执行）。新增 `teams` / `team_messages` 两张表
  （schema v2 → v3，自动升级）、`lib/src/team.js`、`/team` 系列端点、
  `team_open` / `team_say` / `team_task` / `team_status` 四个工具、
  输入栏的「团队模式」下拉、面板的「群组」页签（客户端第 5 片 `045-team-group.js`）。
  四条设计决定：**驱动在宿主**（主对话回合结束不掐断讨论，轮流制由串行循环保证）、
  **群主是唯一调度者**（每次发言后话语权回到群主，它用最后一行 `@名字`/`@用户`/`@等待`/`@收尾` 指挥）、
  **群聊记录是唯一的共享上下文**（每轮按预算注入最近若干条）、
  **讨论与执行是一条闭环**（群主用 `team_task` 边谈边派，任务结算由宿主汇报回群并叫回群主）。
  团队模式下任务链**固定自动激活**（并拒绝在团队模式开着时关掉它）；
  `team_say` 与 `team_status` 进入 `readonly` 白名单（只读成员也能说话），
  `team_task` / `team_open` 不在（那是写操作）。
...
6. **主对话不会被打断通知**：自动激活发生在两次工具调用之间，没有推送通道。
   ...
   团队模式下的执行汇报是**发进群聊**的，主对话要看就调 `team_status`（或看面板的「群组」页签）。
7. **团队模式的讨论是串行的**：一个团队同时只能有一个人在发言（轮流制），
   而派出去的任务与讨论并行（并发仍由 `maxConcurrentRuns` 一处管）。
   轮到的成员正忙时讨论会暂停并说明是谁在忙，等它跑完点「继续」。
8. **群聊记录不参与保留策略的清理**：`retentionDays` 清的是运行记录，
   而 `teams` / `team_messages` 会一直留着（它们是讨论的原始记录，删了就没法回看「当初怎么定的」）。
   团队是按需开的、条数天然有限，但如果要长期高频使用，请留意这张表的增长。
9. **版本兼容性是已知的真实风险**：本插件针对 `@deepseek-ai/dsh@0.1.2-rc.1` 开发，
   依赖的是 DSH 的内部接口。区间外的版本会加载但可能降级 —— 见「版本兼容性」一节，
   以及 `/sub-agent/api/health` 的 `harness` 与 `capabilities` 字段。

> 界面截图拍摄于 `0.1.0`，因此不包含 `0.2.0` 的任务链路图与 `0.3.0` 的群组页签。
