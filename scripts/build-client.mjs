 ...
 *
 * ⚠ **排序有语义**，不只是好看：拼接结果是一个函数体，所以
 *  - `01-head.js` 打开工厂（`window.__ModuleLoader__.load({factory: ...})`），
 *  - 中间的每个分片都必须是**函数/常量声明**（顶层语句会在工厂里立刻执行），
 *  - **最后一片（`05-settings-tail.js`）负责收尾**：关掉工厂，并把 `exports.*` 一次性列出来。
